...
import Interweb from './modules/Interweb.jsx';
import Augment from './modules/Augment.jsx';
import CodeModule from './modules/CodeModule.jsx';
import Scribe from './modules/Scribe.jsx';
import Optics from './modules/Optics.jsx';
import { CANVAS } from './geo.js';
import { renderState } from './api.js';

const STATES = ['idle', 'interweb', 'augment', 'code', 'scribe', 'optics'];
...
export default function App() {
  ...
  const mod = () => {
    switch (mode) {
      case 'interweb': return <Interweb go={go} data={appData.current.interweb} setData={setDataFor('interweb')} />;
      case 'augment': return <Augment go={go} data={appData.current.augment} setData={setDataFor('augment')} />;
      case 'code': return <CodeModule go={go} data={appData.current.code} setData={setDataFor('code')} />;
  ...
  return (
    <div className="stage-wrap" style={{ alignItems: anchorBottom ? 'flex-end' : 'center' }}>
      <div className="stage" data-scale={s}
        ...
        {!booted ? (
          <>
            <video className="boot-video" src="/assets/boot.mp4" autoPlay muted playsInline
