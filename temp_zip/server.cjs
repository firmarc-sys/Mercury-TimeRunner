...
const MERCURY = [
  (q) => `I hold state across every surface, Jorge. "${q}" is threaded into the runtime.`,
  (q) => `Compiled "${String(q).slice(0, 48)}" against the Mercury engine. No divergence.`,
...
function api(req, res, body) {
  ...
  if (u === '/api/tae' && req.method === 'POST') {
    ...
    if (p.op === 'code.chat') { li = (li + 1) % MERCURY.length; return send({ reply: MERCURY[li](p.prompt || '') }); }
    if (p.op === 'interweb.fetch') {
      const host = String(p.url || '').replace(/^https?:\/\//, '').split('/')[0] || 'unknown';
      return send({ result: { url: p.url, host, status: 'MERCURY FETCH // BACKEND', lines: [
        `RESOLVE ${host} :: 12ms`, 'HANDSHAKE :: chrome-tunnel v3', 'RENDER-STATE :: composited',
        'SUMMARY :: Backend surface active. Live crawl intelligence attaches here.' ] } });
    }
    return send({ ok: true });
  }
  if (u === '/api/render-state') return send({ state: 'ok', modules: ['idle','interweb','augment','code','scribe','optics'], ts: Date.now() });
  if (u === '/api/identity') return send({ user: 'Jorge', tier: 'persistent', engine: 'mercury' });
  if (u === '/api/iot') return send({ devices: [], status: 'standby' });
  if (u === '/api/syncori') return send({ sync: 'idle', lastPulse: Date.now() });
...
}).listen(PORT, '0.0.0.0', () => console.log('JA.I Mercury v1 on ' + PORT));
