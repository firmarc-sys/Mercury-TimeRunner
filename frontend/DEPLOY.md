# JAHORIN Mercury (v2) — Cloud Run deployment

From Google Cloud Shell:

    # 1. Upload this zip via Cloud Shell (three-dot menu -> Upload), then:
    unzip jai-mercury-v2-cloudrun.zip -d jai-mercury-v2
    cd jai-mercury-v2
...
- Local test: npm install && npm run build && cp -r public/assets2 public/fonts dist/ && node server.cjs
